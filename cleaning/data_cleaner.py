...
def clean_products(products: List[Product]) -> List[Product]:
    ...
    if 'availability' in df.columns:
        ...
        if mask_price_invalid.any():
            ...
            extracted_price = df.loc[mask_price_invalid, 'availability'].str.extract(price_pattern)[0]
            
            # Normalize decimal separator
            extracted_price = extracted_price.str.replace(',', '.', regex=False)
            
            # Update only invalid prices
            df.loc[mask_price_invalid, 'price'] = pd.to_numeric(extracted_price, errors='coerce').fillna(0.0)
