...
def generate_dashboard(db: DatabaseManager, output_path: str = str(DASHBOARD_HTML_PATH)) -> str:
    ...
    if 'availability' in df.columns:
        in_stock_count = len(df[df['availability'].str.contains('In Stock', case=False, na=False)])
        if total_products > 0:
            pct = (in_stock_count / total_products) * 100
            availability_pct = f"{int(pct)}%"
            if pct > 80: availability_label = "Stock Level Healthy"
            elif pct > 50: availability_label = "Stock Level Moderate"
            else: availability_label = "Stock Level Low"
